...
class AreaBoundaryProcessor:
    ...
    def extract_curves_from_boundary(self, boundary: Base) -> List[Curve]:
        ...
        possible_curve_attrs = ['curves', 'outline', 'boundary', 'geometry', 'displayValue']
    ...
    def is_curve(self, obj: Any) -> bool:
        ...
        return any(curve_type in str(getattr(obj, 'speckle_type', '')) for curve_type in curve_types)
    ...
    def create_surface_from_curves(self, curves: List[Curve], metadata: Dict[str, Any]) -> Base:
    ...
    def extract_metadata(self, boundary: Base) -> Dict[str, Any]:
        ...
        preserve_attrs = [
            'area', 'perimeter', 'name', 'number', 'level', 
            'roomNumber', 'roomName', 'department', 'occupancy',
...
def automate_function(
    ...
    boundaries = [
        ...
        if hasattr(obj, 'speckle_type') and 
        ('AreaBoundary' in str(obj.speckle_type) or 'Area' in str(obj.speckle_type))
    ]
    ...
    for i, boundary in enumerate(boundaries):
        try:
            ...
            if 'area' in metadata and metadata['area'] < function_inputs.min_area_threshold:
                continue
    ...
    if converted_surfaces:
        ...
        automate_context.attach_info_to_objects(
            ...
            object_ids=[surface.id for surface in converted_surfaces if hasattr(surface, 'id')],
        ...
        automate_context.mark_run_failed("No area boundaries could be converted to surfaces")
